...
const state = { checkedAt: null, targets: [], services: [], aiUsage: [], errors: [], history: {} };
...
async function saveHistory() {
  ...
  await writeFile(temporary, JSON.stringify(state.history), 'utf8');
...
async function load(){const r=await fetch('/api/status');window.data=await r.json();const d=window.data;const products=d.targets.filter(x=>x.group==='Products'), others=d.targets.filter(x=>x.group!=='Products');const all=[...products,...others];const up=all.filter(x=>x.up).length, running=d.services.filter(x=>x.up).length;document.getElementById('summary').innerHTML='<div class="metric"><span>Public checks</span><b>'+up+' / '+all.length+'</b></div><div class="metric"><span>Docker runtime</span><b>'+running+' / '+d.services.length+'</b></div><div class="metric"><span>AI sources</span><b>'+d.aiUsage.filter(x=>x.connected).length+' / '+d.aiUsage.length+'</b></div>';document.getElementById('updated').textContent='Last check: '+new Date(d.checkedAt).toLocaleString();render(all,'targets',x=>row(x,(x.statusCode?'HTTP '+x.statusCode+' · ':'')+(x.latencyMs+' ms'),x.up?'UP':'DOWN'));render(d.services,'services',x=>row(x,x.kind+' · '+x.detail,x.up?'RUNNING':'DEGRADED'));render(d.aiUsage,'ai',x=>row({id:x.id,name:x.name,connected:x.connected},x.connected?(x.detail+' · '+x.requests+' requests') : x.detail,x.connected?(x.input+' in / '+x.output+' out'):'CONNECT'));const issues=[...all.filter(x=>!x.up),...d.services.filter(x=>!x.up),...d.aiUsage.filter(x=>!x.connected)];document.getElementById('attention').innerHTML=issues.length?issues.map(x=>'<div class="row"><div><div class="name">'+esc(x.name)+'</div><div class="muted">'+esc(x.detail||'Needs review')+'</div></div><div class="status down">ACTION</div><span></span></div>').join(''):'<div class="empty">No active incidents.</div>'}
...
const server = createServer((request, response) => {
  ...
  if (request.url === '/api/status') return json(response, state);
  if (request.url === '/healthz') return json(response, { ok: true });
